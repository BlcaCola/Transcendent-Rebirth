...
export default (env, argv) => {
  ...
  return {
    ...
    output: {
      ...
      filename: isWatch ? 'inline.js' : 'TranscendentRebirth.js',
      clean: true,
      publicPath: isProduction ? './' : '/', // 生产环境用相对路径，开发环境用绝对路径
    },
    ...
    plugins: [
      ...
      new webpack.DefinePlugin({
        ...
        'BACKEND_BASE_URL': JSON.stringify(process.env.BACKEND_BASE_URL || 'http://127.0.0.1:12345')
      }),
      new HtmlWebpackPlugin({
        template: './index.html',
        inject: isWatch ? false : 'body', // watch 模式不自动注入，其他模式注入到 body
        minify: isProduction ? {
          removeComments: true,
