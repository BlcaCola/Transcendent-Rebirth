  ...
  <div id="app-container">
    ...
    <ActionMenu v-if="!isInGameView" position="top-right" openTitle="功能" closeTitle="关闭">
      <template #menu="{ close }">
        ...
        <button class="action-menu-item" @click="openAPIModal(close)">
          <Plug :size="18" />
          <span>API配置</span>
        </button>
...
import { BookOpen, X, Scroll, Sparkles, Maximize2, Minimize2, Moon, Sun, Settings, Globe, UserCircle, Heart, ArrowRight, Plug, Shield } from 'lucide-vue-next'; // 导入图标
...
const handleLoggedIn = () => {
  ...
  void apiStore.loadFromBackend();
  void characterStore.loadLocalDataFromBackend();
  void loadUserPromptsFromBackend();
  switchView('ModeSelection');
};

const loadUserPromptsFromBackend = async () => {
  if (!backendReady.value) return;
