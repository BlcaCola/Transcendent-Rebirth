  ...
  <div v-if="visible" class="modal-overlay" @click.self="handleClose">
    <div class="modal-dialog">
      ...
      <div v-if="currentStep === 'code'" class="step-content">
        ...
        <p class="modal-subtitle">{{ $t('请输入兑换码以创建联机世界进行AI推演。') }}</p>
...
const getPromptHint = () => {
  const hints = {
    ...
    spirit_root: '描述你希望的改造核心属性、特色或训练倾向（可含对话，使用中文引号“”，每句仅一对）',
    talent: '描述你希望的模块能力、特殊技能或接口（可含对话，使用中文引号“”，每句仅一对）'
  };
...
const getPlaceholderText = () => {
  const placeholders = {
    world: '例如：现代都市赛博世界，信号复苏，科技与改造并存...',
    talent_tier: '例如：擅长硬件改造的天才，拥有超强的感知能力...',
    origin: '例如：企业家族的落魄继承人，身负旧案...',
    spirit_root: '例如：罕见的电磁改造核心，天生抗干扰...',
    talent: '例如：能够看透系统漏洞的侦测模块...'
  };
...
const submitCode = async () => {
  ...
  try {
    await request<{ id: number; code: string; times_used: number; max_uses: number }>(
      `/api/v1/redemption/validate/${code.value.trim()}`,
      { method: 'POST' }
    );
    ...
    const message = err instanceof Error ? err.message : '兑换码验证失败，请检查后重试。';
    error.value = message;
  } finally {
    isValidating.value = false;
  }
