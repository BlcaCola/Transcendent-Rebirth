<template>
  <div class="spirit-root-selection-container">
    <div v-if="store.isLoading" class="loading-state">{{ $t('系统扫描，探查改造核心...') }}</div>
      ...
      <div class="spirit-root-left-panel">
        ...
        <div class="top-actions-container">
          ...
          <button
            ...
            class="action-item shimmer-on-hover"
          >
            <span class="action-name">{{ $t('AI帮你创建') }}</span>
          </button>
        </div>
        ...
        <div v-if="selectionMode === 'preset'" class="preset-mode">
          <div class="spirit-root-list-container">
            ...
            >
              <span class="spirit-root-name">{{ $t('随机改造') }}</span>
        ...
        <div v-if="selectionMode === 'custom'" class="custom-mode">
          <div class="custom-selection-container">
            ...
            <div class="selection-group">
              <label class="selection-label">{{ $t('改造类型') }}</label>
            ...
            <!-- 品级选择 -->
            <div class="selection-group">
              <label class="selection-label">{{ $t('改造品级') }}</label>
            ...
            <div class="custom-preview">
              ...
              <div class="preview-content">
                ...
                <div class="preview-stats">
                  <div class="stat">{{ $t('同步倍率:') }} {{ getCustomSpiritMultiplier() }}x</div>
      ...
      <div class="spirit-root-details-container">
        <div v-if="activeSpiritRoot || (selectionMode === 'custom' && customSpirit.type !== 'none')" class="spirit-root-details">
          ...
          <div class="stats-display">
            <div class="stat-item">
              <span class="stat-label">{{ $t('训练倍率:') }}</span>
              <span class="stat-value">{{ getActiveMultiplier() }}x</span>
            </div>
            <div class="stat-item">
              <span class="stat-label">{{ $t('消耗技能点:') }}</span>
              <span class="stat-value">{{ getActiveCost() }}{{ $t('点') }}</span>
            </div>
          </div>
        </div>
        <div v-else class="placeholder">{{ $t('请选择一种改造核心，或随机选择。') }}</div>
      </div>
    </div>
...
const spiritRootTypes = [
  { key: 'fire', name: '等离子', icon: '🔥', color: '#ef4444', desc: '高能放电，瞬时爆发' },
  { key: 'water', name: '液冷', icon: '💧', color: '#3b82f6', desc: '冷却循环，持续稳定' },
  { key: 'wood', name: '生物', icon: '🧬', color: '#10b981', desc: '生体修复，适应强化' },
  { key: 'metal', name: '合金', icon: '🦾', color: '#f59e0b', desc: '硬化装甲，结构强化' },
  { key: 'earth', name: '地质', icon: '🗿', color: '#8b5cf6', desc: '地形适配，抗压防护' },
  { key: 'wind', name: '气流', icon: '💨', color: '#06b6d4', desc: '气流推进，机动提升' },
  { key: 'thunder', name: '电磁', icon: '⚡', color: '#eab308', desc: '电磁干扰，穿透压制' },
  { key: 'ice', name: '冷凝', icon: '❄️', color: '#0ea5e9', desc: '低温锁定，抑制过载' },
  { key: 'light', name: '光谱', icon: '☀️', color: '#f97316', desc: '光谱成像，识别净化' },
  { key: 'dark', name: '影域', icon: '🌑', color: '#6b7280', desc: '隐匿潜行，干扰侦测' },
  { key: 'space', name: '量子', icon: '🌀', color: '#7c3aed', desc: '量子位移，空间掌控' },
  { key: 'time', name: '时序', icon: '⏰', color: '#ec4899', desc: '时序优化，行动加速' }
]

// 改造品级配置 - 赛博等级体系
const spiritRootTiers = [
  { key: 'common', name: '标准级', multiplier: 1.0, cost: 0, desc: '基础规格，稳定可靠' },
  { key: 'low', name: '增强级', multiplier: 1.1, cost: 3, desc: '性能略优，勉强可用' },
  { key: 'middle', name: '专业级', multiplier: 1.3, cost: 6, desc: '规格尚可，小有成效' },
  { key: 'high', name: '精英级', multiplier: 1.6, cost: 10, desc: '性能出众，前途可期' },
  { key: 'supreme', name: '极限级', multiplier: 2.0, cost: 15, desc: '万中无一，顶级配置' },
  { key: 'heaven', name: '传奇级', multiplier: 2.4, cost: 20, desc: '罕见定制，极其稀有' },
  { key: 'divine', name: '超核级', multiplier: 2.8, cost: 25, desc: '传说级别，近乎不可得' },
  { key: 'special', name: '特例', multiplier: 0, cost: 0, desc: '特殊机型，另有用途' }
]
...
const advancedCustomFields: readonly ModalField[] = [
  { key: 'name', label: '改造名称', type: 'text', placeholder: '例如：混沌改造' },
  { key: 'tier', label: '品级', type: 'select', options: spiritRootTiers.map(t => ({ value: t.key, label: t.name })) },
  { key: 'description', label: '改造描述', type: 'textarea', placeholder: '描述这个改造的特性和背景故事...' },
  { key: 'cultivation_speed', label: '训练速度', type: 'text', placeholder: '例如：极快、快速、普通、缓慢' },
  { key: 'base_multiplier', label: '训练倍率', type: 'number', placeholder: '例如：1.5' },
  { key: 'talent_cost', label: '消耗技能点', type: 'number', placeholder: '例如：10' },
  { key: 'rarity', label: '稀有度', type: 'number', placeholder: '1-10，数值越高越稀有' },
  {
    ...
    columns: [
      {
        key: 'effect',
        placeholder: '效果描述，如：电磁模块输出+80%'
      }
    ]
  }
]

// 为自定义改造数据定义完整类型 - 与标准数据格式保持一致
...
function validateCustomSpiritRoot(data: Partial<CustomSpiritRootData>) {
    ...
    // 必填字段验证
    if (!data.name?.trim()) errors.name = '改造名称不可为空';
    if (!data.tier) errors.tier = '请选择品级';
    if (!data.description?.trim()) errors.description = '改造描述不可为空';
    ...
    if (data.base_multiplier !== undefined && data.base_multiplier !== '' && isNaN(baseMultiplier)) {
      errors.base_multiplier = '同步倍率必须为数字';
    }
...
async function handleCustomSubmit(data: CustomSpiritRootData) {
  ...
  try {
    ...
    toast.success(`自定义改造 "${newRoot.name}" 已保存！`);
  } catch (e) {
    console.error('保存自定义改造失败:', e);
    toast.error('保存自定义改造失败！');
  }
}
...
const activeDisplayName = computed(() => {
 if (activeSpiritRoot.value === 'random') return '随机改造'
...
const activeDescription = computed(() => {
 if (activeSpiritRoot.value === 'random')
   return '系统噪声与规则并存。选择此项，你的改造核心将完全随机生成，可能一步登顶，亦可能平庸无奇。'
 if (activeSpiritRoot.value && typeof activeSpiritRoot.value === 'object') return activeSpiritRoot.value.description || '改造信息不明。'
 return '改造信息不明。'
});
...
async function handleAIPromptSubmit(userPrompt: string) {
  ...
  toast.loading('世界推演中，请稍候...', { id: toastId });
  ...
  try {
    ...
    console.log('【AI推演-改造】完整响应:', aiResponse);
    ...
    try {
      ...
      console.error('【AI推演-改造】JSON解析失败:', parseError);
    ...
    if (!parsedRoot.name && !parsedRoot.名称) {
      toast.error('AI推演结果缺少改造名称', { id: toastId });
      return;
    }
    ...
    const newRoot: SpiritRoot = {
      id: Date.now(),
      name: String(parsedRoot.name || parsedRoot['名称'] || '未命名改造'),
      ...
      base_multiplier: Number(parsedRoot.base_multiplier || parsedRoot['同步倍率']) || 1.0,
      talent_cost: Number(parsedRoot.talent_cost || parsedRoot['技能点消耗'] || parsedRoot['点数消耗']) || 5,
    ...
    toast.success(`AI推演完成！改造 "${newRoot.name}" 已生成`, { id: toastId });

  } catch (e: unknown) {
    console.error('【AI推演-改造】失败:', e);
...
function getCustomSpiritName(): string {
  if (customSpirit.type === 'none') return '请选择改造类型';
  const typeInfo = spiritRootTypes.find(t => t.key === customSpirit.type);
  return typeInfo ? `${typeInfo.name}改造` : '未知改造';
}
...
function confirmCustomSpirit() {
  ...
  if (!typeInfo || !tierInfo) {
    toast.error('选择的改造配置无效');
    return;
  }

  const newRoot: SpiritRoot = {
    id: Date.now(),
    name: `${typeInfo.name}改造`,
  ...
  store.addSpiritRoot(newRoot);
  handleSelectSpiritRoot(newRoot);
  toast.success(`自定义改造 "${newRoot.name}" 已创建！`);
...
async function handleDeleteSpiritRoot(id: number) {
  try {
    ...
    console.log(`【改造选择】成功删除改造 ID: ${id}`);
  } catch (error) {
    console.error(`【改造选择】删除改造失败 ID: ${id}`, error);
  }
}
...
async function handleEditSubmit(data: CustomSpiritRootData) {
  ...
  try {
    ...
    if (success) {
      ...
      toast.success(`改造 "${updateData.name}" 已更新！`);
    } else {
      toast.error('更新改造失败！');
    }
  } catch (e) {
    console.error('更新改造失败:', e);
    toast.error('更新改造失败！');
  }
}
...
.spirit-root-tier.tier-low {
  background: linear-gradient(135deg, #8B5CF6, #A78BFA);
  color: white;
}

.spirit-root-tier.tier-middle {
  background: linear-gradient(135deg, #3B82F6, #60A5FA);
  color: white;
}

.spirit-root-tier.tier-high {
  background: linear-gradient(135deg, #10B981, #34D399);
  color: white;
}

.spirit-root-tier.tier-supreme {
  background: linear-gradient(135deg, #F59E0B, #FBBF24);
  color: white;
}

.spirit-root-tier.tier-heaven {
  background: linear-gradient(135deg, #EF4444, #F87171);
  color: white;
}

.spirit-root-tier.tier-divine {
  background: linear-gradient(135deg, #DC2626, #F87171);
  color: white;
}

.spirit-root-tier.tier-special {
  background: linear-gradient(135deg, #7C3AED, #A78BFA);
  color: white;
  border: 1px solid #A78BFA;
}

.spirit-root-tier.tier-common {
