    ...
    <div v-else class="tier-layout">
      ...
      <div class="tier-left-panel">
        ...
        <div class="top-actions-container">
          ...
          >
            <span class="action-name">{{ $t('自定义人设') }}</span>
          </button>
      ...
      <div class="tier-details-container">
        <div v-if="activeTier" class="tier-details">
          ...
          <div class="points-display">{{ $t('技能点') }}: {{ activeTier.total_points }}</div>
        </div>
        <div v-else class="placeholder">{{ $t('请选择你的人设等级，这将决定你的起点。') }}</div>
      </div>
    </div>
...
const filteredTalentTiers = computed(() => {
  ...
  if (store.isLocalCreation) {
    ...
    console.log("【天资选择】穿越异世界可用天资列表:", availableTiers);
...
const customTierFields = [
  { key: 'name', label: '人设名称', type: 'text', placeholder: '例如：凡人' },
  { key: 'description', label: '人设描述', type: 'textarea', placeholder: '描述此人设的特点...' },
  { key: 'total_points', label: '技能点', type: 'number', placeholder: '例如：20' },
...
function validateCustomTier(data: Partial<CustomTierData>) {
    ...
    if (isNaN(points) || points < 0) errors.total_points = '技能点必须是非负数';
...
async function handleCustomSubmit(data: CustomTierData) {
  ...
  try {
    ...
    toast.success(`自定义人设 "${newTier.name}" 已保存！`);
  } catch (e) {
    console.error('保存自定义人设失败:', e);
    toast.error('保存自定义人设失败！');
  }
}
