  ...
  <div class="character-management-panel" :class="{ fullscreen: isFullscreen }">
    ...
    <div v-if="modalState.show" class="dialog-overlay" @click="handleModalCancel">
      <div class="dialog-box" @click.stop>
        ...
        <div v-if="modalState.travelPoints !== null" class="dialog-points">
          <div class="points-row">
            <span class="points-label">{{ $t('当前剩余') }}:</span>
            <span class="points-value">{{ modalState.travelPoints }}</span>
          </div>
          <div class="points-row">
            <span class="points-label">{{ $t('本次消耗') }}:</span>
            <span class="points-value">{{ modalState.travelCost }}</span>
          </div>
        </div>
        ...
        <div class="dialog-actions">
          <button
            ...
            :disabled="modalState.isBusy"
          >
            {{ $t('取消') }}
          </button>
          <button @click="handleModalConfirm" class="btn-dialog-confirm" :disabled="modalState.isBusy">
            {{ $t('确认') }}
          </button>
        </div>
      </div>
    ...
    <main class="main-content" :class="{ 'fullscreen-content': isFullscreen }">
      ...
      <div v-if="Object.keys(characterStore.rootState.角色列表).length === 0" class="empty-state">
        ...
        <h2>{{ $t('未找到任何存档') }}</h2>
        <p>{{ $t('尚未创建任何存档，请返回主页开启你的冒险之旅') }}</p>
        <div class="empty-actions">
          <button @click="goBack" class="btn-create">{{ $t('踏入超凡新生') }}</button>
...
const modalState = ref({
  ...
  inputValue: '',
  placeholder: '',
  onConfirm: (() => {}) as (() => void | Promise<void>) | ((value: string) => void | Promise<void>),
  onCancel: () => {},
  travelPoints: null as number | null,
...
const fetchTravelPoints = async (): Promise<number> => {
  if (!isBackendConfigured()) {
    throw new Error('后端未配置，无法获取穿越点数');
  }
  ...
  return Number(profile?.travel_points ?? 0);
};

const consumeTravelPoints = async (): Promise<number> => {
  const res = await request.post<{ travel_points: number }>(
    '/api/v1/auth/travel-points/consume',
    { amount: TRAVEL_POINT_COST }
  );
  return Number(res?.travel_points ?? 0);
};

const showTravelConfirm = async (
  title: string,
  message: string,
  ...
  try {
    ...
    if (points < TRAVEL_POINT_COST) {
      showAlert('穿越点数不足', `当前剩余 ${points} 点，无法执行该操作。`);
      return;
    }
    ...
    modalState.value = {
      ...
      placeholder: '',
      travelPoints: points,
      travelCost: TRAVEL_POINT_COST,
      isBusy: false,
      onConfirm: async () => {
        modalState.value.isBusy = true;
        try {
          const remaining = await consumeTravelPoints();
          await onConfirm();
          toast.info(`已扣除${TRAVEL_POINT_COST}点，剩余${remaining}点`);
        } catch (error) {
          console.error('扣减穿越点数失败', error);
          toast.error('扣减穿越点数失败');
        } finally {
          modalState.value.isBusy = false;
          closeModal();
        }
      },
      onCancel: () => {
    ...
    toast.error((error as Error)?.message || '获取穿越点数失败');
    if (onCancel) onCancel();
  }
};

// 暂停登录校验：避免进入"加载存档"时阻断
...
const showAlert = (title: string, message: string, onConfirm?: () => void | Promise<void>) => {
  modalState.value = {
    show: true,
    type: 'alert',
    title,
...
const showPrompt = (title: string, message: string, initialValue = '', placeholder = '', onConfirm: (value: string) => void | Promise<void>, onCancel?: () => void) => {
  modalState.value = {
    show: true,
    type: 'prompt',
    title,
...
const handleModalConfirm = async () => {
  if (modalState.value.type === 'prompt') {
    await (modalState.value.onConfirm as (value: string) => void | Promise<void>)(modalState.value.inputValue);
  } else {
    await (modalState.value.onConfirm as () => void | Promise<void>)();
  }
};
...
const closeModal = () => {
  modalState.value.travelPoints = null;
  modalState.value.travelCost = null;
  modalState.value.isBusy = false;
...
const exportCharacter = async (charId: string) => {
  ...
  try {
    ...
    link.download = `超凡新生-角色-${characterName}-${new Date().toISOString().split('T')[0]}.json`;
...
const exportSingleSave = async (charId: string, slotKey: string, slot: SaveSlot) => {
  ...
  try {
    ...
    link.download = `超凡新生-${saveName}-${new Date().toISOString().split('T')[0]}.json`;
...
const _exportSaves = async () => {
  ...
  try {
    ...
    const link = document.createElement('a');
    link.href = URL.createObjectURL(dataBlob);
    link.download = `超凡新生-${character.角色.名字}-存档备份-${new Date().toISOString().split('T')[0]}.json`;
...
const handleImportFile = async (event: Event) => {
  ...
  try {
    ...
    if (importMode.value === 'saves') {
      ...
      showTravelConfirm(
        '导入角色',
        `导入角色将消耗${TRAVEL_POINT_COST}个穿越点数，确定要导入角色 "${charName}" 吗？`,
        async () => {
          loading.value = true;
          try {
