    ...
    <div v-else class="origin-layout">
      ...
      <div class="origin-details-container">
        <div v-if="activeOrigin" class="origin-details">
          ...
          <div class="cost-display">{{ $t('消耗技能点: {0}').replace('{0}', String(activeCost)) }}</div>
        </div>
        <div v-else class="placeholder">{{ $t('请选择一处出身，或随机选择。') }}</div>
      </div>
    </div>
...
const filteredOrigins = computed(() => {
  ...
  if (store.isLocalCreation) {
    ...
    console.log("【出身选择】穿越异世界可用出身列表:", availableOrigins);
...
const customOriginFields: ModalField[] = [
  ...
  { key: 'talent_cost', label: '技能点消耗', type: 'number', placeholder: '选择此出身需要消耗的技能点，可为负数表示奖励' },
...
async function handleAIPromptSubmit(userPrompt: string) {
  ...
  try {
    ...
    // 解析技能点消耗（支持多种字段名）
    let talentCost = parsedOrigin.talent_cost || parsedOrigin.技能点消耗 || parsedOrigin.消耗技能点;

    // 如果AI没有提供技能点，给予警告并设置默认值
    if (talentCost === undefined || talentCost === null) {
      console.warn('【AI推演-出身】AI未返回技能点消耗字段，使用默认值3');
      toast.warning('AI未设置技能点消耗，已自动设为3点', { id: toastId, duration: 2000 });
      talentCost = 3; // 默认消耗3点，较为合理
    } else {
      // 确保是数字类型
      talentCost = Number(talentCost);
      if (isNaN(talentCost)) {
        console.warn('【AI推演-出身】技能点消耗不是有效数字，使用默认值3');
        talentCost = 3;
      }
    }
