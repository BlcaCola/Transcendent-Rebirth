...
年龄: number (自动计算)
出生日期: {年, 月, 日, 小时?, 分钟?} (只读)
初始六维: {体质, 能源, 算法, 资源感知, 魅力, 心智} (只读，1-10)
成长六维: {体质, 能源, 算法, 资源感知, 魅力, 心智} (可修改，用add)</pre>

                <h5>1.2 属性（可修改）</h5>
                <pre class="code-block">阶位: {名称, 阶段, 当前进度, 下一级所需, 晋升描述}
  → 用set更新整个对象
  → 晋升时必须更新晋升描述
  ...
  "value": {
    ...
    "状态描述": "电量恢复速度提升50%"
            ...
            <section id="dao" class="format-section">
              <h4 class="section-title">2. 专精</h4>
              <div class="section-content">
                <pre class="code-block">流派列表: {专精名: SpecData对象}
  - 专精名: string
  - 描述: string
  - 阶段列表: [{名称, 描述, 突破经验}]
  - 是否解锁: boolean
...
// 增加经验
{
  ...
  - 信用点: {低额, 中额, 高额, 最高额} (用add增减)
  - 物品: {物品ID: Item对象}

物品类型:
  - 装备: {物品ID, 名称, 类型:"装备", 品质, 数量, 描述, 装备增幅, 特殊效果}
  - 模块: {物品ID, 名称, 类型:"模块", 品质, 数量, 描述, 模块效果, 模块技能[], 训练进度, 已解锁技能[]}
  - 消耗品/材料: {物品ID, 名称, 类型:"丹药"|"材料"|"其他", 品质, 数量, 描述, 使用效果?}

品质格式: {quality:"民用"|"改装"|"军规"|"特级"|"禁忌", grade:0-10}</pre>

                <div class="warning-box">
                  <strong>⚠️ 重要：</strong>
                  <ul>
                    <li>训练进度和已解锁技能存储在<strong>角色.背包.物品.{模块ID}</strong>中</li>
                    <li>❌ 错误：修改"角色.训练.训练模块"字段（这是引用槽位，不是进度来源）</li>
                    <li>✅ 正确：修改"角色.背包.物品.{模块ID}.训练进度"</li>
                  </ul>
                </div>

                <div class="example-box">
                  <strong>示例：</strong>
                  <pre class="code-block">// 增加模块训练进度
{
  "action": "add",
  "key": "角色.背包.物品.module_001.训练进度",
  "value": 10
}
...
// 解锁新技能
{
  ...
  - 名字: string
  - 性别: "男"|"女"|"其他"
  - 阶位: {名称, 阶段} (⚠️ NPC阶位是简化结构，严禁添加"当前进度"等玩家专属字段)
  - 与玩家关系: string
  ...
  - 当前内心想法: string (用set实时更新)</pre>
                ...
                <div class="warning-box">
                  <strong>⚠️ NPC阶位格式：</strong>
                  <ul>
                    <li>✅ 正确：{"名称": "核心级", "阶段": "中期"}</li>
                    <li>❌ 错误：{"名称": "核心级", "阶段": "中期", "当前进度": 0}</li>
                  </ul>
                </div>
...
// 添加记忆
{
  "action": "push",
  "key": "关系.张三.记忆",
  "value": "【1000年3月15日】与玩家一起训练，感觉很愉快"
}
...
势力信息字段:
  ...
  - 成员数量: {总数, 按阶位, 按职位}</pre>
                ...
                <div class="example-box">
                  <strong>示例：</strong>
                  <pre class="code-block">// 修改负责人
{
  "action": "set",
  "key": "世界.势力信息[0].领导层.负责人",
  "value": "新负责人名字"
}
...
事件对象结构:
  ...
  - 事件名称: string
  - 事件类型: string (组织冲突/世界变化/稀有物资现世/封锁区现世/好友遭遇等)
  - 事件描述: string
  - 影响等级: "轻微"|"中等"|"重大"|"灾难"
  - 影响范围: string
  - 相关人物: string[]
