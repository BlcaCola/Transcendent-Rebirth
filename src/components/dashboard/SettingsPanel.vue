      ...
      <div class="settings-section">
        ...
        <div class="settings-list">
          ...
          <div class="setting-item setting-item-full" v-if="currentPlayerName">
            <div class="setting-info">
              <label class="setting-name">{{ t('修改用户名') }}</label>
...
const updatePlayerName = async () => {
  ...
  try {
    ...
    toast.success(`用户名已修改为「${newPlayerName.value}」`);
    newPlayerName.value = ''; // 清空输入框
  } catch (error) {
    console.error('修改用户名失败:', error);
    toast.error('修改用户名失败，请重试');
  }
};
...
const exportSettings = () => {
  ...
  try {
    ...
    link.download = `超凡新生-设置备份-${dateStr}.json`;
