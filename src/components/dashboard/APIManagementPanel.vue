...
onMounted(async () => {
  ...
  await apiStore.loadFromBackend();
  loadAIServiceConfig();
  loadLocalSettings();
...
const testAPI = async (api: APIConfig) => {
  ...
  try {
    ...
    const testPrompt = '你正在进行API连通性测试。请仅输出：超凡新生本-连通测试-OK';
    ...
    const ok = response.toLowerCase().includes('超凡新生本') || response.toLowerCase().includes('ok');
...
const handleExport = () => {
  ...
  link.href = url;
  link.download = `超凡新生-API配置-${new Date().toISOString().split('T')[0]}.json`;
  link.click();
