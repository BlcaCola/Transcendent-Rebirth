...
const exportSingleSave = async (save: SaveSlot) => {
  try {
    ...
    const fileName = `超凡新生-${save.存档名}-${new Date().toISOString().split('T')[0]}.json`;
...
const exportCharacter = async () => {
  try {
    ...
    const fileName = `超凡新生-角色-${characterName}-${new Date().toISOString().split('T')[0]}.json`;
    link.download = fileName;
...
const exportSaves = async () => {
  try {
    ...
    const link = document.createElement('a');
    link.href = URL.createObjectURL(dataBlob);
    const fileName = `超凡新生-存档备份-${new Date().toISOString().split('T')[0]}.json`;
    link.download = fileName;
