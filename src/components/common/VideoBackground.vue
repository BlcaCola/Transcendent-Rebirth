...
const loadVideo = async (originalUrl: string) => {
  if (!videoRef.value) {
    console.warn('[VideoBackground] video元素未准备好，延迟重试')
    // 等待下一个tick后重试
    await new Promise(resolve => setTimeout(resolve, 100))
    if (!videoRef.value) {
      console.error('[VideoBackground] video元素仍未准备好，放弃加载')
      return
    }
  }

  // 在加载新视频前，如果存在旧的 object URL，先释放掉
  if (videoRef.value.src && videoRef.value.src.startsWith('blob:')) {
  ...
  try {
    ...
    if (cachedVideo) {
      ...
      if (videoRef.value) videoRef.value.src = objectURL
    } else {
      // 直接尝试从原始 URL 获取
