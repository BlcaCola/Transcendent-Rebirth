/**
 * @fileoverview 世界事件系统规范
 *
 * 说明：
 * - 本文件用于指导 GM（AI）如何理解“世界会变动、会发生大事件”
 * - 随机事件的调度由前端代码负责（见 eventSystem 数据结构与 AIBidirectionalSystem 的注入逻辑）
 */

export const EVENT_SYSTEM_RULES = `
# 规则：世界事件（重要）

## 核心理念
- 世界不以玩家为中心：公司、势力、黑市、系统波动、资源与人心都会自行演变
- 大事件必须影响现实：至少带来一个可感知后果（危险、资源、关系、地缘格局、训练环境变化等）
- 事件必须“刚刚发生”：叙事中用现场感呈现，而不是公告式总结

## 事件类型示例（可混合）
- 公司火并/势力冲突（围剿、清洗、同盟、割据）
- 系统变革（信号潮汐、主机异常、规则松动、封锁解除）
- 异物出现（黑市货物流出、古老设备苏醒、协议现身）
- 区域事件（入口出现、规则变化、限时开放）
- 人物风波（好友受伤、好友升级、好友被追杀、恩怨升级）

## 合理性要求
- 事件强度需与玩家等级、声望、位置、当前状态相符（低等级可卷入边缘，仍须受波及）
- 若涉及好友/NPC，优先使用存档中的“关系”与“好感度”推断其动机与结局
- 事件后续需留下“悬念/尾巴”，推动玩家下一步行动
`.trim();

