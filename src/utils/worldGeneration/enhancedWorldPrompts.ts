...
- 等级体系：根据世界背景选择合适的等级体系（如街头人/跑者/潜影者等）
- 势力类型：根据世界背景选择合适的势力类型（如公司、帮派、走私联盟、安保集团等）
...
### 势力分布参考
公司(35-45%) | 帮派(20-30%) | 走私联盟(10-20%) | 情报组织(10-15%) | 安保集团(5-15%) | 黑客团体(5-10%)
...
\`\`\`json
{
  ...
  "按等级": {
    // 🚨 等级不能超过领导层.最高等级
    // 等级：街头人 < 跑者 < 潜影者 < 雇佣猎手 < 战术大师 < 企业特使 < 幽网主宰 < 城邦执掌 < 传说级
    "跑者": 数量,
    "潜影者": 数量
  },
  "按职位": {
...
### 6种标准类型（全部使用中文）
1. 城市天际线 - 都市地标（摩天楼、巨构）
2. 城区集散地 - 城市聚居地（商业街、交通枢纽）
3. 核心节点 - 高价值区域（数据中心、核心机房）
4. 资源节点 - 资源产地（矿区、回收站）
5. 高危区域 - 危险区域（黑网裂隙、辐射区）
6. 其他特殊 - 特殊地点（遗迹、封存入口）

### 特殊属性（${finalSecretRealmCount}个）
- 高价值节点: ${opportunityRealms}个
- 遗留设施: ${heritageRealms}个
- 高危禁区: ${dangerousRealms}个
  ...
  "locations": [
    {
      "id": "loc_1",
      "名称": "地点名称",
      "类型": "城区/商会/黑市/禁区/险地/避难所",
      "坐标": {"x": 2500, "y": 1500},
      // ⚠️ 地点坐标使用游戏坐标系统 (${minX}-${maxX})，不是经纬度
      // ⚠️ 地点坐标必须在对应大洲边界内
      // ⚠️ 类型必须是中文：城区/商会/黑市/禁区/险地/避难所/其他特殊
      "描述": "地点详细描述",
      "安全等级": "安全",
      "适合阶位": ["街区级以上"],
      "所属势力": "霓虹议会",
