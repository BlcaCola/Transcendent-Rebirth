...
const redirectToLogin = () => {
  ...
  void import('@/stores/apiManagementStore').then(({ useAPIManagementStore }) => {
    const apiStore = useAPIManagementStore();
    void apiStore.clearAll();
  });
  void import('@/stores/characterStore').then(({ useCharacterStore }) => {
    const characterStore = useCharacterStore();
    void characterStore.clearLocalCharacterData();
  });
  void import('@/services/promptStorage').then(({ promptStorage }) => {
    void promptStorage.resetAll();
  });
...
export async function fetchCharacterProfile(charId: string): Promise<unknown> {
  try {
    ...
    toast.error('获取更新角色数据失败，请检查网络或联系管理员。');
    throw error;
  }
}
