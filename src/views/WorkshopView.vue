...
const downloadAsFile = () => {
  ...
  link.download = `超凡新生-工坊-${downloadModal.value.item.type}-${dateStr}.json`;
...
const loadLocalSettings = () => {
  try {
    ...
    uploadPayload.value = {
      settings,
      exportInfo: { timestamp: new Date().toISOString(), version: versionLabel.value, gameVersion: `超凡新生 v${versionLabel.value}` },
    };
...
const submitUpload = async () => {
  ...
  try {
    ...
    await createWorkshopItem({
      ...
      game_version: `超凡新生 v${versionLabel.value}`,
      data_version: '1',
    });
    toast.success('上传成功');
    uploadTitle.value = '';
