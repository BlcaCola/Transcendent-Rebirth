    ...
    <div class="admin-panel">
      ...
      <template v-else>
        <div v-if="!isAdmin" class="locked">
          <p>需要管理员权限才能访问后端管理。</p>
          <p class="hint">请在登录页面勾选"管理员登录"后重新登录。</p>
          <div class="actions">
            <button class="btn btn-secondary" @click="goBack">返回</button>
        ...
        <template v-else>
          ...
          <div class="content">
            ...
            <div v-if="activeTab === 'saves'" class="section">
              ...
              <div class="toolbar">
                <input v-model="saveQuery" class="input" placeholder="搜索角色名/用户名..." />
            ...
            <div v-else-if="activeTab === 'characters'" class="section">
              ...
              <div class="toolbar">
                <input v-model="characterQuery" class="input" placeholder="搜索角色名/用户名..." />
            ...
            <div v-else-if="activeTab === 'invitation_codes'" class="section">
              <div class="section-head">
                <div class="section-title">邀请码管理</div>
                <div class="section-sub">创建与查看邀请码</div>
              </div>

              <div class="toolbar">
                <input v-model="invitationForm.custom_code" class="input" placeholder="自定义邀请码（可选）" />
                <input
                  v-model.number="invitationForm.max_uses"
                  class="input small"
                  type="number"
                  min="-1"
                  placeholder="可用次数(-1无限)"
                />
                <input
                  v-model="invitationForm.days_valid"
                  class="input small"
                  type="number"
                  min="0"
                  placeholder="有效天数(可选)"
                />
                <button class="btn" :disabled="isBusy" @click="createInvitationCode">创建</button>
                <button class="btn btn-secondary" :disabled="isBusy" @click="loadInvitationCodes">加载</button>
              </div>

              <div v-if="invitationError" class="error">{{ invitationError }}</div>
              <div v-else-if="invitationLoading" class="loading">加载中…</div>
              <div v-else class="table codes">
                <div class="row header">
                  <div class="cell id">ID</div>
                  <div class="cell name">邀请码</div>
                  <div class="cell">状态</div>
                  <div class="cell num">已用/上限</div>
                  <div class="cell time">到期</div>
                  <div class="cell time">创建时间</div>
                  <div class="cell actions">操作</div>
                </div>
                <div v-for="c in invitationCodes" :key="c.id" class="row">
                  <div class="cell id">#{{ c.id }}</div>
                  <div class="cell name">{{ c.code }}</div>
                  <div class="cell">
                    <span class="pill" :class="{ banned: !c.is_active }">{{ c.is_active ? '启用' : '停用' }}</span>
                  </div>
                  <div class="cell num">{{ c.times_used }} / {{ c.max_uses < 0 ? '∞' : c.max_uses }}</div>
                  <div class="cell time">{{ c.expires_at ? formatDate(c.expires_at) : '-' }}</div>
                  <div class="cell time">{{ c.created_at ? formatDate(c.created_at) : '-' }}</div>
                  <div class="cell actions">
                    <button class="btn sm" :disabled="isBusy" @click="toggleInvitationCode(c)">
                      {{ c.is_active ? '停用' : '启用' }}
                    </button>
                    <button class="btn sm danger" :disabled="isBusy" @click="deleteInvitationCode(c)">删除</button>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div v-else-if="activeTab === 'codes'" class="section">
              <div class="section-head">
                ...
                <div class="section-sub">创建与查看兑换码</div>
              </div>

              <div class="toolbar">
                <input v-model="redemptionForm.custom_code" class="input" placeholder="自定义兑换码（可选）" />
                ...
                />
                <input
                  v-model.number="redemptionForm.max_uses"
                  class="input small"
                  type="number"
                  min="-1"
                  placeholder="可用次数"
                />
                <input
                  v-model="redemptionForm.days_valid"
                  class="input small"
                  type="number"
                  min="0"
                  placeholder="有效天数(可选)"
                />
                <button class="btn" :disabled="isBusy" @click="createRedemptionCode">创建</button>
                <button class="btn btn-secondary" :disabled="isBusy" @click="loadRedemptionCodes">加载</button>
              </div>

              <div v-if="redemptionError" class="error">{{ redemptionError }}</div>
              <div v-else-if="redemptionLoading" class="loading">加载中…</div>
              <div v-else class="table codes">
                <div class="row header">
                  <div class="cell id">ID</div>
                ...
                <div v-for="c in redemptionCodes" :key="c.id" class="row">
                  <div class="cell id">#{{ c.id }}</div>
                  <div class="cell name">{{ c.code }}</div>
                  <div class="cell">{{ c.reward_type }} × {{ c.reward_value }}</div>
                  <div class="cell num">{{ c.times_used }} / {{ c.max_uses < 0 ? '∞' : c.max_uses }}</div>
                  <div class="cell time">{{ c.expires_at ? formatDate(c.expires_at) : '-' }}</div>
                  <div class="cell time">{{ c.created_at ? formatDate(c.created_at) : '-' }}</div>
                  <div class="cell actions">
                    <button class="btn sm danger" :disabled="isBusy" @click="deleteRedemptionCode(c)">删除</button>
                  </div>
                </div>
              </div>
            </div>
...
const invitationCodes = ref<InvitationCodeRow[]>([]);
const invitationForm = reactive({
  custom_code: '',
  max_uses: -1,
  days_valid: '' as string | number,
});

const redemptionLoading = ref(false);
const redemptionError = ref('');
const redemptionCodes = ref<RedemptionCodeRow[]>([]);
...
const loadInvitationCodes = async () => {
  ...
  try {
    const res = await adminRequest.get<{ total: number; items: InvitationCodeRow[] }>('/api/v1/admin/invitation-codes');
    invitationCodes.value = res?.items || [];
  } catch (e: any) {
    invitationError.value = e?.message || '加载失败';
  } finally {
...
const createInvitationCode = async () => {
  ...
  try {
    const payload: Record<string, any> = {
      max_uses: Number(invitationForm.max_uses ?? -1),
    };
    if (invitationForm.custom_code.trim()) payload.custom_code = invitationForm.custom_code.trim();
    if (String(invitationForm.days_valid).trim()) payload.days_valid = Number(invitationForm.days_valid);
    await adminRequest.post('/api/v1/admin/invitation-codes', payload);
    toast.success('邀请码已创建');
    invitationForm.custom_code = '';
    invitationForm.days_valid = '';
...
const deleteInvitationCode = async (item: InvitationCodeRow) => {
  if (!confirm(`确定删除邀请码 ${item.code}？`)) return;
  isBusy.value = true;
  try {
    await adminRequest.delete(`/api/v1/admin/invitation-codes/${item.id}`);
...
const toggleInvitationCode = async (item: InvitationCodeRow) => {
  isBusy.value = true;
  try {
    await adminRequest.patch(`/api/v1/admin/invitation-codes/${item.id}`, {
      is_active: !item.is_active,
    });
    toast.success(item.is_active ? '已停用邀请码' : '已启用邀请码');
    await loadInvitationCodes();
  } catch (e: any) {
...
const loadRedemptionCodes = async () => {
  ...
  try {
    const res = await adminRequest.get<{ total: number; items: RedemptionCodeRow[] }>('/api/v1/admin/redemption-codes');
    redemptionCodes.value = res?.items || [];
  } catch (e: any) {
    redemptionError.value = e?.message || '加载失败';
  } finally {
...
const createRedemptionCode = async () => {
  ...
  try {
    const payload: Record<string, any> = {
      max_uses: Number(redemptionForm.max_uses ?? 1),
      reward_type: redemptionForm.reward_type || 'ai',
      reward_value: Number(redemptionForm.reward_value ?? 1),
    };
    if (redemptionForm.custom_code.trim()) payload.custom_code = redemptionForm.custom_code.trim();
    if (String(redemptionForm.days_valid).trim()) payload.days_valid = Number(redemptionForm.days_valid);
    await adminRequest.post('/api/v1/admin/redemption-codes', payload);
    toast.success('兑换码已创建');
    redemptionForm.custom_code = '';
    redemptionForm.days_valid = '';
...
const deleteRedemptionCode = async (item: RedemptionCodeRow) => {
  if (!confirm(`确定删除兑换码 ${item.code}？`)) return;
  isBusy.value = true;
  try {
    await adminRequest.delete(`/api/v1/admin/redemption-codes/${item.id}`);
...
const refreshActive = async () => {
  ...
  } else if (activeTab.value === 'invitation_codes') {
    await loadInvitationCodes();
