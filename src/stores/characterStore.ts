...
import { request, createCharacter as createCharacterAPI, fetchCharacterProfile, updateCharacterSave, verifyStoredToken } from '@/services/request';
...
export const useCharacterStore = defineStore('characterV3', () => {
  ...
  const commitMetadataToStorage = async (): Promise<void> => {
    try {
      ...
      scheduleSyncLocalDataToBackend();
  ...
  // === 本地存档数据（角色列表/存档JSON）同步 ===
  let syncTimer: number | null = null;

  const buildLocalDataSnapshot = async () => {
    const characters = JSON.parse(JSON.stringify(rootState.value.角色列表 || {}));
    const saves: Record<string, Record<string, SaveData>> = {};

    for (const [charId, profile] of Object.entries(characters)) {
      const slots = profile?.存档列表 ? Object.keys(profile.存档列表) : [];
      if (!slots.length) continue;
      saves[charId] = {};
      await Promise.all(
        slots.map(async (slotId) => {
          const saveData = await storage.loadSaveData(charId, slotId);
          if (saveData) {
            saves[charId][slotId] = saveData;
          }
        })
      );
    }

    return { characters, saves };
  };

  const saveLocalDataToBackend = async () => {
    const token = localStorage.getItem('access_token');
    if (!token || !isBackendConfigured()) return;
    try {
      const payload = await buildLocalDataSnapshot();
      await request.put('/api/v1/user/local-data', payload);
    } catch (error) {
      debug.error('角色商店', '同步本地存档数据到后端失败', error);
    }
  };

  const scheduleSyncLocalDataToBackend = () => {
    const token = localStorage.getItem('access_token');
    if (!token || !isBackendConfigured()) return;
    if (syncTimer) {
    ...
    syncTimer = window.setTimeout(() => {
      syncTimer = null;
      void saveLocalDataToBackend();
    }, 1200);
  };

  const clearLocalCharacterData = async () => {
    rootState.value = { 当前激活存档: null, 角色列表: {} };
    await storage.saveCharacters({});
    await storage.saveActiveSave(null);
    await storage.deleteAllSaveData();
  };

  const loadLocalDataFromBackend = async () => {
    const token = localStorage.getItem('access_token');
    if (!token || !isBackendConfigured()) return false;
    try {
      const data = await request.get<{ characters: Record<string, CharacterProfile> | null; saves: Record<string, Record<string, SaveData>> | null }>(
        '/api/v1/user/local-data'
      );
      if (!data || !data.characters || !data.saves) return false;

      await clearLocalCharacterData();

      rootState.value.角色列表 = data.characters || {};
      rootState.value.当前激活存档 = null;

      await storage.saveCharacters(rootState.value.角色列表);
      await storage.saveActiveSave(null);

      for (const [charId, slots] of Object.entries(data.saves || {})) {
        for (const [slotId, saveData] of Object.entries(slots || {})) {
          await storage.saveSaveData(charId, slotId, saveData as SaveData);
        }
      }
      ...
      debug.error('角色商店', '从后端导入本地存档数据失败', error);
      return false;
    }
  };
  ...
  const createNewCharacter = async (payload: CreationPayload): Promise<CharacterBaseInfo | undefined> => {
    ...
    try {
      ...
      try {
        ...
        if (mode === '单机') { // 单机
          console.log('[角色商店] 穿越异世界,尝试离线初始化...');
  ...
  const loadGame = async (charId: string, slotKey: string) => {
      ...
      try {
        ...
        uiStore.updateLoadingText('世界重置完毕，正在加载存档...');
...
return {
  ...
  saveLocalDataToBackend,
  loadLocalDataFromBackend,
