...
- 死亡风险极高，每一步都需谨慎`
  ...
  const bonusTalentPoints = computed(() => {
    ...
    if (selectedTalents.value.some(t => t.name === '霸王血脉')) {
      points += 1;
      console.log('[技能点计算] 检测到 "霸王血脉" 天赋, 增加 1 技能点');
    }
    return points;
  });
  ...
  const remainingTalentPoints = computed(() => {
    ...
    console.log('[技能点计算] 初始技能点:', points);

    // Add bonus points from talents
    points += bonusTalentPoints.value;

    if (selectedOrigin.value) {
      console.log('[技能点计算] 出生消耗:', selectedOrigin.value.talent_cost);
    ...
    console.log('[技能点计算] 已选天赋数量:', selectedTalents.value.length);
    console.log('[技能点计算] 已选天赋列表:', selectedTalents.value.map(t => ({ 名称: t.name, 消耗: t.talent_cost })));
    console.log('[技能点计算] 天赋总消耗:', talentCost);
    points -= talentCost;
  ...
  async function initializeStore(currentMode: 'single' | 'cloud') {
    ...
    try {
      ...
      console.log("【创世神殿】已获取用户用户名:", characterPayload.value.character_name);
    } catch (error) {
      console.warn('【创世神殿】获取用户用户名失败，使用默认名', error);
      characterPayload.value.character_name = '无名者';
    }

    try {
      if (currentMode === 'single') {
        console.log("【创世神殿】初始化穿越异世界，加载本地数据和自定义数据！");
        ...
        console.log("【创世神殿】初始化联机模式，从后端获取更新数据！");
        ...
        if (isBackendConfigured()) {
          ...
          if (!tokenValid) {
            ...
            try {
              ...
              console.error("【创世神殿】获取更新数据失败，回退到本地数据:", fetchError);
              // 回退到本地数据
              creationData.value.worlds = LOCAL_WORLDS.map(w => ({ ...w, source: 'local' as DataSource }));
              creationData.value.talentTiers = LOCAL_TALENT_TIERS.map(t => ({ ...t, source: 'local' as DataSource }));
              creationData.value.origins = LOCAL_ORIGINS.map(o => ({ ...o, source: 'local' as DataSource }));
  ...
  async function fetchCloudWorlds() {
    ...
    try {
      ...
      console.log("【创世神殿】成功获取更新世界数据", cloudWorlds);
      ...
      error.value = `获取更新世界列表失败: ${e instanceof Error ? e.message : '未知错误'}`;
  ...
  async function fetchAllCloudData(): Promise<number> {
    ...
    try {
      ...
      console.error("【创世神殿】获取更新创世数据失败", e);
      error.value = `获取更新创世数据失败: ${e instanceof Error ? e.message : '未知错误'}`;
