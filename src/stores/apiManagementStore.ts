...
export type APIUsageType =
  ...
  | 'sect_generation';  // 组织内容生成（资料库、权限商店等）
...
export const useAPIManagementStore = defineStore('apiManagement', () => {
  const buildDefaultAPI = (): APIConfig => ({
    id: 'default',
    name: '默认API',
    provider: 'openai',
  ...
  // API配置列表
  const apiConfigs = ref<APIConfig[]>([buildDefaultAPI()]);
  ...
  const resetToDefaults = () => {
    apiConfigs.value = [buildDefaultAPI()];
    apiAssignments.value = [...DEFAULT_API_ASSIGNMENTS];
    functionModes.value = [...DEFAULT_FUNCTION_MODES];
    functionEnabled.value = [...DEFAULT_FUNCTION_ENABLED];
    aiGenerationSettings.value = { enableSystemCoT: false };
  };
  ...
  const applyConfig = (data: any) => {
    ...
    if (apiConfigs.value.length === 0) {
      apiConfigs.value.push(buildDefaultAPI());
    }
  };
  ...
  const loadFromBackend = async () => {
    ...
    try {
      const data = await request.get<{ config: any | null }>('/api/v1/user/api-config');
      if (data?.config) {
        applyConfig(data.config);
      } else {
        resetToDefaults();
      }
      saveToStorage({ skipRemote: true });
      return true;
    } catch (error) {
      console.error('[API管理] 从后端加载配置失败:', error);
      resetToDefaults();
      return false;
    }
  };

  // 保存到localStorage（缓存）并同步到后端
  const saveToStorage = (options?: { skipRemote?: boolean }) => {
    try {
      const token = localStorage.getItem('access_token');
      if (!token) {
        localStorage.removeItem('api_management_config');
        return;
      }
  ...
  const saveToBackend = async (payload?: ReturnType<typeof buildConfigPayload>) => {
    const token = localStorage.getItem('access_token');
    if (!token || !isBackendConfigured()) return;
    try {
      const data = payload ?? buildConfigPayload();
      await request.put('/api/v1/user/api-config', { config: data });
    } catch (error) {
      console.error('[API管理] 保存到后端失败:', error);
    }
  };

  const clearAll = async (options?: { clearRemote?: boolean }) => {
    resetToDefaults();
    localStorage.removeItem('api_management_config');
    const token = localStorage.getItem('access_token');
    if (options?.clearRemote && token && isBackendConfigured()) {
      try {
        await request.delete('/api/v1/user/api-config');
      } catch (error) {
        console.error('[API管理] 清空后端配置失败:', error);
      }
    }
  };
